...
def test_callback_rejects_invalid_state() -> None:
    ...
    response = client.get("/callback?code=code&state=bad")

    assert response.status_code == 400


def test_callback_exchanges_code_and_stores_session(monkeypatch) -> None:
    client = client_for(FakeApp())
    ...
    assert "template_session" in callback.cookies
