...
Database tables are managed by asyncpg-backed migrations:

```bash
task db:update                 # apply migrations
task db:migration -- add table # create a manual migration revision
```

Run these commands with the same `APP_DATABASE__*` Postgres settings the app
uses. Applied revisions are tracked in the `schema_migrations` table.
